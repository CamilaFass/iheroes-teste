...
class HeroesTable extends Component {
  ...
  fillEditForm(row, id) {
    let inputName = document.querySelector(
      '#hero-edit-form .hero-inputbox input[name="name"]'
    );
    let inputClass = document.querySelector(
      '#hero-edit-form .hero-inputbox input[name="class"]'
    );
    let inputId = document.querySelector(
      '#hero-edit-form .hero-inputbox input[name="_id"]'
    );
          ...
          <div>
            <a>
              <img
                ...
                src={editIcon}
                onClick={() => this.initEdit(item._id)}
              />
            </a>
          </div>
          <div>
    ...
    return (
      <div className="hero-table-container">
        <Table strip bordered hover>
          ...
          <tbody>{heroesList}</tbody>
        </Table>
        <Pagination
          data={this.props.data.heroes}
          callback={this.props.changeHeroesPage}
          page="heroesPage"
        />
      </div>
    );
