...
class OccurrencesTable extends Component {
  ...
  fillOccurrencesTable() {
    ...
    occurrences = getPaginationGroup(
      this.props.data.occurrencesPage,
      this.props.data.occurrences
    );
    if (occurrences.length) {
      let data = occurrences.map((item) => {
        let log = generateDesignatedHeroList(
          this.props.data.heroesLog,
          item._id
        );
        if (log == undefined) {
          log = "";
    ...
    return (
      <div className="hero-table-container">
        <Table strip bordered hover>
          ...
          <tbody>{this.fillOccurrencesTable()}</tbody>
        </Table>
        <Pagination
          data={this.props.data.occurrences}
          callback={this.props.changeOccurrencesPage}
          page="occurrencesPage"
        />
      </div>
    );
  }
...
const mapDispatchToProps = (dispatch) =>
  bindActionCreators({ changeOccurrencesPage }, dispatch);
