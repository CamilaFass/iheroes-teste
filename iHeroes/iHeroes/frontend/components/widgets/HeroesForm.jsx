...
class HeroesForm extends Component {
  ...
  getInputFormValues(form) {
    let inputName = document.querySelector(
      `${form} .hero-inputbox input[name="name"]`
    );
    let inputClass = document.querySelector(
      `${form} .hero-inputbox input[name="class"]`
    );
    let inputId = document.querySelector(
      `${form} .hero-inputbox input[name="_id"]`
    );
  ...
  generateCancelButton() {
    if (!this.props.heroes.createForm) {
      let button = (
        <Button variant="primary" onClick={this.cancelEdit}>
          Cancelar
        </Button>
      );
      return button;
  ...
  renderForm() {
    if (this.props.heroes.createForm) {
      let createForm = this.generateForm(
        "Digite os dados do herói para adicionar",
        "hero-add-form",
        "flex",
        this.submitCreate
      );
      ...
      let editForm = this.generateForm(
        "Digite os dados do herói para editar",
        "hero-edit-form",
        "flex",
        this.submitEdit
      );
...
const mapDispatchToProps = (dispatch) =>
  bindActionCreators({ getHeroData, changeCreateFormView }, dispatch);
