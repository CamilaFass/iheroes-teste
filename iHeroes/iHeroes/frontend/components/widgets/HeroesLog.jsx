...
class HeroesLog extends Component {
  ...
  fillHeroesLogTable() {
    let data = getPaginationGroup(
      this.props.data.heroesLogPage,
      this.props.data.heroesLog
    );
    if (data.length > 1) {
      let content = data.map((item) => {
        let occurrence = this.props.data.occurrences.filter((reg) =>
          filterOccurrences(reg, item.occurrence)
        )[0];
        return (
          <tr>
    ...
    return (
      <div className="hero-table-container">
        <Table strip bordered hover>
          ...
          <tbody>{this.fillHeroesLogTable()}</tbody>
        </Table>

        <Pagination
          data={this.props.data.heroesLog}
          callback={this.props.changeHeroesLogPage}
          page="heroesLogPage"
        />
      </div>
    );
...
const mapDispatchToProps = (dispatch) =>
  bindActionCreators({ changeHeroesLogPage }, dispatch);
